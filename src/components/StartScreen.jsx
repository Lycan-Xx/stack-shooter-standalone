...
export default function StartScreen({ onStartGame, onStartTutorial }) {
  ...
  useEffect(() => {
    ...
    setProfile({ bestWave: Math.max(...difficulties.map(getBestWave)), totalKills: lifetime.totalKills, difficulty: savedSettings.difficulty || 'normal' });
  }, []);
  ...
  const updateSetting = (updates) => {
    ...
    setSettings(next);
    gameStorage.setSettings(updates);
  ...
  return (
    <main id="start-screen" className="lobby-shell">
      <header className="lobby-topbar"><div className="lobby-brand"><GameMark size={34} /><div><span className="lobby-brand__eyebrow">STACK SHOOTER</span><h1>Vampire Siege</h1></div></div><IconButton icon="settings" label="Settings" onClick={() => setShowSettings(true)} /></header>
      {tab === 'play' ? <div className="lobby-content">
      ...
      </div> : <div className="lobby-content rankings-tab"><div className="rankings-heading"><p className="section-label">PERFORMANCE CENTER</p><h2>Rankings</h2><p>Every record and lifetime stat in one place.</p></div><section className="lobby-stats"><StatRow label="Best wave" value={profile.bestWave} accent="primary" /><StatRow label="Total kills" value={formatNumber(profile.totalKills)} accent="secondary" /><StatRow label="Last mode" value={profile.difficulty} accent="gold" /></section><GlassSurface className="rankings-preview rankings-preview--full"><div className="lobby-section__heading"><p className="section-label">LOCAL RECORDS</p><span>TOP RUNS</span></div>{hasScores ? difficulties.map((difficulty) => (highScores[difficulty] || []).map((score, index) => <div className="ranking-line" key={`${difficulty}-${index}`}><span>{difficulty} · #{index + 1}</span><strong>{score.score.toLocaleString()}</strong><small>WAVE {score.wave}</small></div>)) : <p className="rankings-empty">Complete a run to establish your first record.</p>}</GlassSurface></div>}
      <nav className="lobby-bottom-nav" aria-label="Main navigation"><button className={tab === 'play' ? 'is-active' : ''} onClick={() => setTab('play')}><GameIcon name="play" size={21} /><span>PLAY</span></button><button onClick={() => setShowHowToPlay(true)}><GameIcon name="shield" size={21} /><span>ARMORY</span></button><button className={tab === 'rankings' ? 'is-active' : ''} onClick={() => setTab('rankings')}><GameIcon name="trophy" size={21} /><span>RANKINGS</span></button><button onClick={() => setShowSettings(true)}><GameIcon name="settings" size={21} /><span>SETTINGS</span></button></nav>
      ...
      {showSettings && <OverlaySheet title="Settings" kicker="SYSTEM PREFERENCES" onClose={() => setShowSettings(false)} className="lobby-settings"><div className="pause-settings"><label><span>Sound effects <output>{settings.soundEnabled !== false ? 'ON' : 'OFF'}</output></span><input type="checkbox" checked={settings.soundEnabled !== false} onChange={(e) => updateSetting({ soundEnabled: e.target.checked })} /></label><label><span>Effects volume <output>{Math.round(settings.soundVolume * 100)}%</output></span><input type="range" min="0" max="1" step="0.05" value={settings.soundVolume} onChange={(e) => updateSetting({ soundVolume: Number(e.target.value) })} /></label><label><span>Music volume <output>{Math.round(settings.musicVolume * 100)}%</output></span><input type="range" min="0" max="1" step="0.05" value={settings.musicVolume} onChange={(e) => updateSetting({ musicVolume: Number(e.target.value) })} /></label></div><GameButton onClick={() => setShowSettings(false)}>Done</GameButton></OverlaySheet>}
    </main>
  );
}
